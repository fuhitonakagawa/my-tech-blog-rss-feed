...
## サイトの追加方法
[src/resources/feed-info-list.ts](https://github.com/fuhitonakagawa/my-tech-blog-rss-feed/blob/main/src/resources/feed-info-list.ts) の `FEED_SECTION_LIST` でセクションごとに管理しており、その一覧にない場合 issue を作っていただければ対応します。
...
3. フィードを追加
   `src/resources/feed-info-list.ts` の `FEED_SECTION_LIST` の対象セクションを更新
4. コミット
...
- `src/resources/feed-info-list.ts` のセクション・ブログ情報
